...
proc newInput*[W: static int, T](pixels: varargs[array[W, T]]): auto =
  TestInput[W, T](pixels: pixels.toSeq)

proc width*[W: static int, T](image: TestInput[W, T]): int =
  W

proc height*[W: static int, T](image: TestInput[W, T]): int =
  image.pixels.len

proc getPixel*[W: static int, T](image: TestInput[W, T], x, y: int): T =
  image.pixels[y][x]

proc newOutput*[N: static int, T](): auto =
  OutputImage[N, T](pixels: newSeq[array[N, T]]())
...
proc `==`*[N: static int, M: static int, T](
    img: var OutputImage[N, T], values: openarray[array[M, T]]
): bool =
  when N == M:
    return img.pixels == values.toSeq
  else:
    return false
