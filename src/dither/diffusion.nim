...
iterator relativeNumerators(
    diffusor: Diffusor, x, y, maxX, maxY: int
): tuple[x, y, numerator: int] =
  ## Yields the relative coordinates and numerator for a diffusor
...
proc slot[E](errors: var seq[seq[E]], x, y: int): var E =
  errors[y mod errors.len][x]
...
proc errorDiffusionDither*[C, E](
  ...
  for y in 0 ..< input.height:
    for x in 0 ..< input.width:
      ...
      for (errorX, errorY, numerator) in diffusor.relativeNumerators(
        x, y, input.width, input.height
      ):
        errors.slot(errorX, errorY) =
          errors.slot(errorX, errorY) + (error * numerator div errorDenominator)
